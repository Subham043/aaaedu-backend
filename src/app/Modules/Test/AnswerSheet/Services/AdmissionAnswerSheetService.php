...
class AdmissionAnswerSheetService
{
    public function apply_test(Test $test, AdmissionTest $admissionInfo): TestTaken
    {
        $quiz = (new QuizService)->getCurrentQuizByTestId($test->id);
        $payment_arr = array();
        $payment_arr['receipt'] = $admissionInfo->receipt;
        $payment_arr['amount'] = $admissionInfo->amount;
        $payment_arr['razorpay_order_id'] = $admissionInfo->razorpay_order_id;
        $payment_arr['razorpay_payment_id'] = $admissionInfo->razorpay_payment_id;
        $payment_arr['razorpay_signature'] = $admissionInfo->razorpay_signature;
        $payment_arr['payment_status'] = PaymentStatus::PAID->value;
        ...
        $test_taken = TestTaken::create(
            [
                ...
                'is_enrolled' => true,
                'enrollment_type' => TestEnrollmentType::PURCHASED->value,
